...
void initDrawing() {
	...
	gBookkeepingData.mFrequency = counter.tv_sec + (counter.tv_nsec / 1e9);
...
static void startDrawingBookkeeping()
{
	struct timespec counter;
    clock_gettime(CLOCK_MONOTONIC, &counter);
    gBookkeepingData.mDrawingStartTime = counter.tv_sec + (counter.tv_nsec / 1e9);
}
...
static void stopDrawingBookkeeping()
{
    struct timespec counter;
    clock_gettime(CLOCK_MONOTONIC, &counter);
    gBookkeepingData.mDrawingEndTime = counter.tv_sec + (counter.tv_nsec / 1e9);
}
...
void resetDrawingFrameStartTime() {
    struct timespec counter;
    clock_gettime(CLOCK_MONOTONIC, &counter);
    gBookkeepingData.mFrameStartTime = counter.tv_sec + (counter.tv_nsec / 1e9);
