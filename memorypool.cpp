// adapted from https://github.com/eapache/wof_alloc
...
#define WOF_FREE_HEADER_SIZE sizeof(wof_free_hdr_t)
	...
	* replaces the input chunk in whichever list it originally inhabited. */
			...
			const size_t size)
	{
		...
		* with our new chunk header) we can safely write our new chunk header. */
		...
		extra->used = FALSE;
		extra->jumbo = FALSE;
		extra->allocator = (long int)allocator;
	...
	* adds that chunk to the free list. */
			...
			wof_block_hdr_t* block)
	{
		...
		chunk->allocator = (long int)allocator;
	...
	void* callocPoolMemory(void* tPool, size_t nelem, size_t elsize) {
		...
		void* data = allocPoolMemory(tPool, size);
		memset(data, 0, size);
		return data;
	}
	...
	int isMemoryInPool(void* tPool, void* ptr) {
		...
		wof_chunk_hdr_t* chunk;
		chunk = WOF_DATA_TO_CHUNK(ptr);
		return chunk->allocator == (long int)allocator;
	}
